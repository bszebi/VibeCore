...
try {
    ...
    $befejezve_id = mysqli_query($conn, "SELECT id FROM maintenance_status WHERE name = 'Befejezve' LIMIT 1");
    $torolve_id = mysqli_query($conn, "SELECT id FROM maintenance_status WHERE name = 'Törölve' LIMIT 1");
    
    if (!$befejezve_id || !$torolve_id) {
    ...
    if (!$befejezve_row || !$torolve_row) {
        throw new Exception("Státuszok nem találhatók");
    }

    // Aktív karbantartások számának lekérdezése
